...
export default function StudentDetailPage({
  ...
  params: Promise<{ batch: string; degree: string; id: string }>;
}) {
  // Unwrap params using React.use()
  const { batch, degree, id } = use(params);
  const router = useRouter();

  // Use hooks
  const { data: student, isLoading, error } = useStudent(id, batch, degree);
  ...
  // Construct back URL with batch and degree context
  const backUrl = `/${batch}/${degree}/students`;
