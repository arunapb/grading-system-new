...
export function PDFTable({ pdfs, onRefresh, batch, degree }: PDFTableProps) {
  ...
  const handleParseAll = async () => {
    const missingCount = pdfs.filter((p: PDFInfo) => !p.hasOutput).length;
  ...
  const handleBatchParse = async (
    ...
    if (successCount > 0) {
      toast.success(
        `Successfully parsed ${successCount} PDF${successCount !== 1 ? "s" : ""}!`,
      );
      onRefresh();
    }
    ...
    if (failCount > 0) {
      toast.error(
        `Failed to parse ${failCount} PDF${failCount !== 1 ? "s" : ""}`,
      );
    }
  };
  ...
  return (
    <div className="space-y-4">
        ...
        <Table>
          ...
          <TableBody>
            {sortedPDFs.length === 0 ? (
              ...
              sortedPDFs.map((pdf: PDFInfo) => (
                <TableRow key={pdf.pdfPath}>
                  <TableCell className="font-medium">
                    {editingCell?.path === pdf.pdfPath &&
                    editingCell?.field === "moduleCode" ? (
                      <div className="flex items-center gap-1">
                        ...
                        <Button
                          ...
                          variant="ghost"
                          className="h-8 w-8"
                          onClick={() => saveEdit(pdf.pdfPath, "moduleCode")}
                        >
                    ...
                    editingCell?.field === "moduleName" ? (
                      <div className="flex items-center gap-1">
                        ...
                        <Button
                          ...
                          variant="ghost"
                          className="h-8 w-8"
                          onClick={() => saveEdit(pdf.pdfPath, "moduleName")}
                        >
                          <Check className="h-4 w-4" />
                        </Button>
                        <Button
                    ...
                    editingCell?.field === "credits" ? (
                      <div className="flex items-center gap-1">
                        ...
                        <Button
                          ...
                          variant="ghost"
                          className="h-8 w-8"
                          onClick={() => saveEdit(pdf.pdfPath, "credits")}
                        >
                          <Check className="h-4 w-4" />
                        </Button>
                        <Button
                      ...
                      <span
                        onClick={() =>
                          startEditing(pdf.pdfPath, "credits", pdf.credits)
                        }
                        className="cursor-pointer hover:bg-accent px-2 py-1 rounded"
                      >
      ...
      <BatchParseDialog
        ...
        pdfs={pdfs.filter((p: PDFInfo) => !p.hasOutput)}
