 ...
 */
export function extractMetadata(relativePath: string): {
  year: string;
  semester: string;
} {
 ...
 * Assumes format: CODE_Module_Name_Rest.pdf
 * e.g., "IN3410_Data_Mining_and_Data_Warehousing_IT_Intake_2021_...pdf" -> "Data_Mining_and_Data_Warehousing"
 */
export function extractModuleName(filename: string): string {
  ...
  if (parts.length >= 2) {
    // Find where the name ends. Usually before degree keywords like IT, ITM, AI, Intake, Semester
    const keywords = ["IT", "ITM", "AI", "INTAKE", "SEMESTER", "BATCH", "YEAR"];
    let endIndex = parts.length;

    for (let i = 1; i < parts.length; i++) {
      const partUpper = parts[i].toUpperCase();
      // Check if part starts with any keyword (e.g. "Intake 2021")
      // Also check if part *is* a degree code like IT, AI (exact match preference or boundary)
      if (keywords.some((k) => partUpper.startsWith(k) || partUpper === k)) {
        endIndex = i;
        break;
      }
    }

    // If we found a keyword, the name is everything from part 1 up to (but not including) the keyword
    // Otherwise, if no keyword found, we'll take at least the second part (legacy behavior)
    if (endIndex > 1) {
      return parts.slice(1, endIndex).join("_").trim();
    }
